...
namespace ICE.Capa_Datos.Acciones
{
    public class GestionarInformeDA : IGestionarInformeDA
    {
        ...
        {
            var informesPendientes = await _context.Informes
                .Include(i => i.DatosDeLinea)
                .Include(i => i.DatosGenerales)
                .Include(i => i.Teleproteccion)
                .Include(i => i.DistanciaDeFalla)
                .Include(i => i.TiemposDeDisparo)
                .Include(i => i.CorrientesDeFalla)
                .Where(i => i.SubestacionId == subestacionId && i.Estado == 0)
                .ToListAsync();

            return informesPendientes.Select(informeBD => new Informe
            {
                Id = informeBD.Id,
                Tipo = informeBD.Tipo,
                SubestacionId = informeBD.SubestacionId,
        ...
        public async Task<List<Informe>> ObtenerInformesPorSubestacion(int subestacionId)
        {
            var informesPendientes = await _context.Informes
                ...
                .Include(i => i.TiemposDeDisparo)
                .Include(i => i.CorrientesDeFalla)
                .Where(i => i.SubestacionId == subestacionId)
                .ToListAsync();
